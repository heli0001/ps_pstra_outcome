...
normalfunc = function(dataid,simdata,iter=100,burn=0){
  set.seed(dataid)
  ...
  for (k in 2:iter){
    ...
    for (cl in 1:m){
      ...
      L_cl = k_cl/w_cl
      omega_cl = diag(w_cl)
      cl_Sigma = 1/(1/lasso_sigsq[k-1,cl]+sum(w_cl))
    ...
    for (cl in 1:m){
      ...
      cl_Sigma11 = 1/(1/lasso_sigsq[k-1,cl]+n_cl)
    ...
    for (cl in 1:m){
      ...
      n_cl = dim(data_cl_all)[1]
      data_cl = cbind(data_cl_all$x1,data_cl_all$x2)  #x1,x2
      cl_Sigma10 = 1/(1/lasso_sigsq[k-1,cl]+n_cl)
    ...
    for (cl in 1:m){
      ...
      n_cl = dim(data_cl_all)[1]
      data_cl = cbind(data_cl_all$x1,data_cl_all$x2)  #x1,x2
      cl_Sigma00 = 1/(1/lasso_sigsq[k-1,cl]+n_cl)
    ...
    for(cl in 1:m){
      ...
      var_zetay11 = 1/(1/lasso_sigsq[k-1,cl]+n_cl_11/sigsq11_pos[k])
    ...
    for(cl in 1:m){
      ...
      var_zetay10 = 1/(1/lasso_sigsq[k-1,cl]+n_cl_10/sigsq10_pos[k])
    ...
    for(cl in 1:m){
      ...
      var_zetay00 = 1/(1/lasso_sigsq[k-1,cl]+n_cl_00/sigsq00_pos[k])
    ...
    for(cl in 1:m){
      ...
      var_zetay01 = 1/(1/lasso_sigsq[k-1,cl]+n_cl_01/sigsq01_pos[k])
    ...
    # 4-1 lasso_sigsq
    for(i in 1:m){
      lasso_rate = zeta[k,i]^2+zeta_00[k,i]^2+zeta_10[k,i]^2+zeta_11[k,i]^2+
        zeta_y_00[k,i]^2+zeta_y_01[k,i]^2+zeta_y_10[k,i]^2+zeta_y_11[k,i]^2
  ...
  return(list(gamma_pos=gamma_pos,var_gamma=var_gamma,beta00_pos=beta00_pos,beta10_pos=beta10_pos,beta11_pos=beta11_pos,var_beta00=var_beta00,
              ...
              sigsq00_pos=sigsq00_pos,sigsq01_pos=sigsq01_pos,sigsq10_pos=sigsq10_pos,sigsq11_pos=sigsq11_pos,lasso_sigsq=lasso_sigsq,
...
poster_normal_res = mclapply(1,normalfunc,simdata,iter=iter,burn=burn,mc.cores = 1)

###########################################################################
######################### effect estimand #################################
###########################################################################

effect_normal_func = function(dataid,simdata,poster_res,iter=iter,burn=burn){
  set.seed(dataid)
  data_obs = simdata[[dataid]]$data_obs
  N = dim(data_obs)[1]
