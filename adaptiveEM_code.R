...
ENadpfunc = function(dataid,simdata,iter=100,burn=0){
  set.seed(dataid)
  ...
  lambdasq1 = matrix(-99,nrow=iter,ncol=m)
  lambda2 = matrix(-99,nrow=iter,ncol=m)
  ...
  ## hyper lasso parameters
  lambdasq1[1,] = rgamma(m,shape=1,rate=0.1)
  lambda2[1,] = rgamma(m,shape=1,rate=0.1)
  ...
  for (k in 2:iter){
    ...
    for (cl in 1:m){
      # generate tau_i 
      mean_tau_ps = sqrt(lambdasq1[k-1,cl]*lasso_sigsq[k-1,cl]/zeta[k-1,cl]^2)
      invtau_ps = rinvgauss(1,mean=mean_tau_ps,dispersion = 1/lambdasq1[k-1,cl])
      tau2_ps[cl] = 1/invtau_ps
    }
    ## generate sigma_zeta
    sigma_zeta = diag(0,m,m)
    diag(sigma_zeta) = 1/(1/tau2_ps+lambda2[k-1,cl])
    
    # update zeta
    for (cl in 1:m){
      data_ps = as.matrix(data_obs[data_obs$clu==cl,3:4]) #x1,x2
      n_cl = dim(data_ps)[1]
      w_cl = apply(data_ps%*%gamma_pos[k,]+zeta[k-1,cl],1,rpg,n=1,h=1)
      k_cl = data_obs[data_obs$clu==cl,]$z-1/2
      L_cl = k_cl/w_cl
      omega_cl = diag(w_cl)
      cl_Sigma = 1/(1/(lasso_sigsq[k-1,cl]*sigma_zeta[cl,cl])+sum(w_cl))
      cl_mu = cl_Sigma*(rep(1,n_cl)%*%omega_cl%*%(L_cl-data_ps%*%gamma_pos[k,]))
    ...
    for (cl in 1:m){
      # generate tau_11
      mean_tau11 = sqrt(lambdasq1[k-1,cl]*lasso_sigsq[k-1,cl]/(zeta_11[k-1,cl]^2))
      invtau11 = rinvgauss(1,mean=mean_tau11,dispersion = 1/lambdasq1[k-1,cl])
      tau2_11[cl] = 1/invtau11
    }
    
    ## generate sigma_zeta11
    sigma_zeta11 = diag(0,m,m)
    diag(sigma_zeta11) = 1/(1/tau2_11+lambda2[k-1,])
    ...
    for (cl in 1:m){
      ...
      cl_Sigma11 = 1/(1/(lasso_sigsq[k-1,cl]*sigma_zeta11[cl,cl])+n_cl)
    ...
    for (cl in 1:m){
      # generate tau_10
      mean_tau10 = sqrt(lambdasq1[k-1,cl]*lasso_sigsq[k-1,cl]/(zeta_10[k-1,cl]^2))
      invtau10 = rinvgauss(1,mean=mean_tau10,dispersion = 1/lambdasq1[k-1,cl])
      tau2_10[cl] = 1/invtau10
    }
    
    ## generate sigma_zeta10
    sigma_zeta10 = diag(0,m,m)
    diag(sigma_zeta10) = 1/(1/tau2_10+lambda2[k-1,])
    ...
    for (cl in 1:m){
      ...
      n_cl = dim(data_cl_all)[1]
      data_cl = cbind(data_cl_all$x1,data_cl_all$x2)  #x1,x2
      cl_Sigma10 = 1/(1/(lasso_sigsq[k-1,cl]*sigma_zeta10[cl,cl])+n_cl)
    ...
    for (cl in 1:m){
      # generate tau_00
      mean_tau00 = sqrt(lambdasq1[k-1,cl]*lasso_sigsq[k-1,cl]/(zeta_00[k-1,cl]^2))
      invtau00 = rinvgauss(1,mean=mean_tau00,dispersion = 1/lambdasq1[k-1,cl])
      tau2_00[cl] = 1/invtau00
    }
    
    ## generate sigma_zeta00
    sigma_zeta00 = diag(0,m,m)
    diag(sigma_zeta00) = 1/(1/tau2_00+lambda2[k-1,])
    ...
    for (cl in 1:m){
      ...
      n_cl = dim(data_cl_all)[1]
      data_cl = cbind(data_cl_all$x1,data_cl_all$x2)  #x1,x2
      cl_Sigma00 = 1/(1/(lasso_sigsq[k-1,cl]*sigma_zeta00[cl,cl])+n_cl)
    ...
    for(cl in 1:m){
      # generate tau_y11
      mean_tauy11 = sqrt(lambdasq1[k-1,cl]*lasso_sigsq[k-1,cl]/(zeta_y_11[k-1,cl]^2))
      invtauy11 = rinvgauss(1,mean=mean_tauy11,dispersion = 1/lambdasq1[k-1,cl])
      tau2_y11[cl] = 1/invtauy11
    }
    
    ## generate sigma_zeta_y11
    sigma_zetay11 = diag(0,m,m)
    diag(sigma_zetay11) = 1/(1/tau2_y11+lambda2[k-1,])
    ...
    for (cl in 1:m){
      ...
      var_zetay11 = 1/(1/(lasso_sigsq[k-1,cl]*sigma_zetay11[cl,cl])+n_cl_11/sigsq11_pos[k])
    ...
    for(cl in 1:m){
      # generate tau_y10
      mean_tauy10 = sqrt(lambdasq1[k-1,cl]*lasso_sigsq[k-1,cl]/(zeta_y_10[k-1,cl]^2))
      invtauy10 = rinvgauss(1,mean=mean_tauy10,dispersion = 1/lambdasq1[k-1,cl])
      tau2_y10[cl] = 1/invtauy10
    }
    
    ## generate sigma_zeta_y10
    sigma_zetay10 = diag(0,m,m)
    diag(sigma_zetay10) = 1/(1/tau2_y10+lambda2[k-1,])
    
    # update
    for (cl in 1:m){
      data_cl_10 = as.matrix(data_obs3[data_obs3$clu==cl&data_obs3$strata_num==2,c(3,4,5,7)])
      n_cl_10 = dim(data_cl_10)[1]
      var_zetay10 = 1/(1/(lasso_sigsq[k-1,cl]*sigma_zetay10[cl,cl])+n_cl_10/sigsq10_pos[k])
    ...
    for(cl in 1:m){
      # generate tau_y00
      mean_tauy00 = sqrt(lambdasq1[k-1,cl]*lasso_sigsq[k-1,cl]/(zeta_y_00[k-1,cl]^2))
      invtauy00 = rinvgauss(1,mean=mean_tauy00,dispersion = 1/lambdasq1[k-1,cl])
      tau2_y00[cl] = 1/invtauy00
    }
    
    ## generate sigma_zeta_y00
    sigma_zetay00 = diag(0,m,m)
    diag(sigma_zetay00) = 1/(1/tau2_y00+lambda2[k-1,])
    
    # update
    for (cl in 1:m){
      data_cl_00 = as.matrix(data_obs3[data_obs3$clu==cl&data_obs3$strata_num==3,c(3,4,5,7)])
      n_cl_00 = dim(data_cl_00)[1]
      var_zetay00 = 1/(1/(lasso_sigsq[k-1,cl]*sigma_zetay00[cl,cl])+n_cl_00/sigsq00_pos[k])
    ...
    for(cl in 1:m){
      # generate tau_y01
      mean_tauy01 = sqrt(lambdasq1[k-1,cl]*lasso_sigsq[k-1,cl]/(zeta_y_01[k-1,cl]^2))
      invtauy01 = rinvgauss(1,mean=mean_tauy01,dispersion = 1/lambdasq1[k-1,cl])
      tau2_y01[cl] = 1/invtauy01
    }
    
    ## generate sigma_zeta_y01
    sigma_zetay01 = diag(0,m,m)
    diag(sigma_zetay01) = 1/(1/tau2_y01+lambda2[k-1,])
    
    # update
    for (cl in 1:m){
      data_cl_01 = as.matrix(data_obs3[data_obs3$clu==cl&data_obs3$strata_num==4,c(3,4,5,7)])
      n_cl_01 = dim(data_cl_01)[1]
      var_zetay01 = 1/(1/(lasso_sigsq[k-1,cl]*sigma_zetay01[cl,cl])+n_cl_01/sigsq01_pos[k])
    ...
    # 4-2 lasso_sigsq
    for (i in 1:m){
      lasso_rate = zeta[k,i]^2*(1/sigma_zeta[i,i])+zeta_00[k,i]^2*(1/sigma_zeta00[i,i])+
        zeta_10[k,i]^2*(1/sigma_zeta10[i,i])+zeta_11[k,i]^2*(1/sigma_zeta11[i,i])+
        zeta_y_00[k,i]^2*(1/sigma_zetay00[i,i])+zeta_y_01[k,i]^2*(1/sigma_zetay01[i,i])+
        zeta_y_10[k,i]^2*(1/sigma_zetay10[i,i])+zeta_y_11[k,i]^2*(1/sigma_zetay11[i,i])
    ...
    # 4-1 lambdasq1
    for (i in 1:m){
      lambda1_rate = tau2_ps[i]+tau2_00[i]+tau2_10[i]+tau2_11[i]+tau2_y00[i]+
        tau2_y10[i]+tau2_y11[i]+tau2_y01[i]
      lambda2_rate = zeta[k,i]^2+zeta_00[k,i]^2+zeta_10[k,i]^2+zeta_11[k,i]^2+zeta_y_00[k,i]^2+
        zeta_y_10[k,i]^2+zeta_y_11[k,i]^2+zeta_y_01[k,i]^2
      
      lambdasq1[k,i] = rgamma(1,shape=8+1,rate=0.1+lambda1_rate/2)
      lambda2[k,i] = rgamma(1,shape=8/2+1,rate=0.1+lambda2_rate/(2*lasso_sigsq[k,i]))
    }
  ...
  return(list(gamma_pos=gamma_pos,var_gamma=var_gamma,beta00_pos=beta00_pos,beta10_pos=beta10_pos,beta11_pos=beta11_pos,var_beta00=var_beta00,
              ...
              sigsq00_pos=sigsq00_pos,sigsq01_pos=sigsq01_pos,sigsq10_pos=sigsq10_pos,sigsq11_pos=sigsq11_pos,lambdasq1=lambdasq1,lambda2=lambda2,lasso_sigsq=lasso_sigsq,
...
poster_enadp_res = mclapply(1:2,ENadpfunc,simdata,iter=iter,burn=burn,mc.cores=1)

###########################################################################
######################### effect estimand #################################
###########################################################################

effect_enadp_func = function(dataid,simdata,poster_res,iter=iter,burn=burn){
  set.seed(dataid)
  data_obs = simdata[[dataid]]$data_obs
  N = dim(data_obs)[1]
...
effect_enadp_res = mclapply(1:2,effect_enadp_func,simdata,poster_enadp_res,iter=iter,burn=burn,mc.cores=1)
